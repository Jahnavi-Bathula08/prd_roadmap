...
def dataset_name_exists(name: str) -> bool:
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT 1 FROM behavior_table WHERE dataset_name = ? LIMIT 1", (name,)
    )
    row = cur.fetchone()
    if not row:
        cur.execute(
            "SELECT 1 FROM feedback_table WHERE dataset_name = ? LIMIT 1", (name,)
        )
        row = cur.fetchone()
    con.close()
...
def save_to_db(df_behavior, df_feedback, name: str):
    ...
    if df_behavior is not None:
        ...
        df_b.columns = [
            "dataset_name", "saved_at", "user_id", "timestamp",
            "event", "category", "severity", "platform", "region", "session_sec"
        ]
    ...
    if df_feedback is not None:
        ...
        df_f.columns = [
            "dataset_name", "saved_at", "user_id", "timestamp",
            "feedback", "category", "priority", "age_group", "platform", "rating"
        ]
...
# ─── JSON persistence (survive refresh) ──────────────────────────────────────
...
if "save_error_msg" not in st.session_state:
...
with st.sidebar:
    ...
    # Show saved dataset names in sidebar
...
if data_exists:
    ...
    # ─── Save input panel (appears inline when Save is clicked) ───────────────
    st.markdown("---")
    ...
    if st.session_state.show_save_input:
        ...
        with save_col1:
            dataset_name = st.text_input(
                "Enter Dataset Name",
                placeholder="e.g. sprint_1_data, march_test_run ...",
    ...
    # ─── Save + Clear buttons at the bottom ───────────────────────────────────
